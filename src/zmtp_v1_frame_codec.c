...
struct zmtp_v1_frame_codec {
    protocol_engine_t base;
    zmtp_v1_frame_encoder_t *encoder;
    zmtp_v1_frame_encoder_info_t encoder_info;
    zmtp_v1_frame_decoder_t *decoder;
    zmtp_v1_frame_decoder_info_t decoder_info;
};

typedef struct zmtp_v1_frame_codec zmtp_v1_frame_codec_t;

static struct protocol_engine_ops ops;

static zmtp_v1_frame_codec_t *
s_new ()
{
    zmtp_v1_frame_codec_t *self =
        (zmtp_v1_frame_codec_t *) malloc (sizeof *self);
    if (self) {
        *self = (zmtp_v1_frame_codec_t) {
            .base.ops = ops,
        };

        self->encoder = zmtp_v1_frame_encoder_new (&self->encoder_info);
        self->decoder = zmtp_v1_frame_decoder_new (&self->decoder_info);

        if (self->encoder == NULL || self->decoder == NULL) {
            zmtp_v1_frame_encoder_destroy (&self->encoder);
            zmtp_v1_frame_decoder_destroy (&self->decoder);
            free (self);
            self = NULL;
        }
    }
...
{
    zmtp_v1_frame_codec_t *self = (zmtp_v1_frame_codec_t *) base;
    assert (self);
...
s_encode (protocol_engine_t *base, pdu_t *pdu, protocol_engine_info_t *info)
{
    ...
    const int rc =
        zmtp_v1_frame_encoder_putmsg (self->encoder, pdu, encoder_info);
    if (rc == -1)
        return -1;
...
s_decode (protocol_engine_t *base, protocol_engine_info_t *info)
{
    ...
    pdu_t *pdu =
        zmtp_v1_frame_decoder_getmsg (self->decoder, decoder_info);
    if (pdu == NULL)
        return pdu;
...
s_read (protocol_engine_t *base, iobuf_t *iobuf, protocol_engine_info_t *info)
{
    ...
    const int rc =
        zmtp_v1_frame_encoder_read (self->encoder, iobuf, encoder_info);
    if (rc == -1)
        return -1;
...
s_read_advance (protocol_engine_t *base, size_t n, protocol_engine_info_t *info)
{
    ...
    const int rc =
        zmtp_v1_frame_encoder_advance (self->encoder, n, encoder_info);
    if (rc == -1)
        return -1;
...
s_write (protocol_engine_t *base, iobuf_t *iobuf, protocol_engine_info_t *info)
{
    ...
    const int rc =
        zmtp_v1_frame_decoder_write (self->decoder, iobuf, decoder_info);
    if (rc == -1)
        return -1;
...
s_write_advance (protocol_engine_t *base, size_t n, protocol_engine_info_t *info)
{
    ...
    const int rc =
        zmtp_v1_frame_decoder_advance (self->decoder, n, decoder_info);
    if (rc == -1)
        return -1;
